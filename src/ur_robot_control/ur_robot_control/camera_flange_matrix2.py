...
class AprilTagBaseNode(Node):
    ...
    # def get_T_flange_tool7(self):
    #     T = np.array([
    #         [0.0, 0.0, 1.0,  0.170094],
    #         [1.0, 0.0, 0.0, -0.009688],
    #         [0.0, 1.0, 0.0,  0.000656],
    #         [0.0, 0.0, 0.0,  1.0]
    #     ])
    ...
    def calculate_camera_aligned_tool7_pose(
        ...
        """
        根據 AprilTag 姿態計算 Base → Tool7。

        姿態處理方式：
            1. 取出 AprilTag 的 Euler 角 rx、ry、rz
            2. 保留 rx、ry
            3. 將 rz 改成：

                rz - TARGET_RZ_OFFSET

            4. 將此姿態當作期望的 Camera optical 姿態
            5. 根據 Tool7 → Camera optical 的固定轉換，
            反推出 Base → Tool7
        """
        ...
        tag_rotation = R.from_matrix(
            T_base_tag[:3, :3]
        )

        try:
            rx, ry, rz = tag_rotation.as_euler(
                'xyz',
                degrees=True
            )
            ...
            raise ValueError(
                f'Tag rotation conversion failed: {exc}'
            ) from exc
        ...
        # Camera rx = Tag rx
        # Camera ry = Tag ry
        # Camera rz = Tag rz - TARGET_RZ_OFFSET
        # ------------------------------------------------

        camera_target_euler_deg = np.array(
            [
                float(rx),
                float(ry),
                float(rz) 
        ...
        camera_target_rotation = R.from_euler(
            'xyz',
            camera_target_euler_deg,
            degrees=True
        )
        ...
        #
        # 位置先使用 AprilTag 的位置
        # 姿態使用 rz offset 修正後的姿態
        # ------------------------------------------------

        T_base_camera_target = np.eye(4)

        T_base_camera_target[:3, :3] = (
            camera_target_rotation.as_matrix()
        )
        ...
        if T_tool7_camera.shape != (4, 4):
            raise ValueError(
                'T_tool7_camera must be a 4x4 matrix'
            )
        ...
        return {
            ...
            'camera_target_euler_deg': (
                camera_target_euler_deg.tolist()
            )
        }
    ...
    def apriltag_callback(self, msg):
        ...
        try:
            ...
            # 計算相機正對 AprilTag 時，
            # Tool7 在 Base 座標中應有的位姿
            camera_aligned_tool7_pose = (
                self.calculate_camera_aligned_tool7_pose(
                    T_base_tag
                )
            )
        ...
        data = {
            ...
            # 相機正對 AprilTag 時，
            # Tool7 在 Base 中應有的完整位姿
            'camera_aligned_tool7_pose_base': (camera_aligned_tool7_pose),
