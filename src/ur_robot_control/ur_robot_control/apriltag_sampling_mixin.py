...
class AprilTagSamplingMixin:
    ...
    def reset_tag_sampling(self):
        ...
        self.camera_aligned_tool_pose_samples = []
    ...
    #         if len(self.latest_tag_position_m) != 3:
    #             raise ValueError(
    ...
    #         if len(self.latest_tag_rj45_position_m) != 3:
    #             raise ValueError(
    #                 'rj45_position_m must contain 3 values'
    #             )

    #         # ====================================================
    #         # Board center Base 座標
    #         # ====================================================

    #         raw_board_center = data.get(
    #             'board_center_base_position_m'
    #         )

    #         if raw_board_center is None:
    #             raise ValueError(
    ...
    #             float(value)
    #             for value in raw_board_center[:3]
    #         ]

    #         if (
    #             len(self.latest_board_center_position_m)
    #             != 3
    #         ):
    #             raise ValueError(
    #                 'board_center_base_position_m '
    ...
    #         # ====================================================
    #         # Camera aligned Tool7 / TCP Pose
    #         # ====================================================

    #         raw_camera_aligned_pose = data.get(
    #             'camera_aligned_tool7_pose_base'
    #         )

    #         if raw_camera_aligned_pose is None:
    #             raise ValueError(
    #                 'camera_aligned_tool7_pose_base '
    #                 'is missing'
    #             )
    ...
    #         ):
    #             raise ValueError(
    #                 'camera aligned position invalid'
    #             )

    #         if (
    #             orientation_quat is None
    #             or len(orientation_quat) != 4
    #         ):
    #             raise ValueError(
    #                 'camera aligned quaternion invalid'
    #             )
    ...
    #             'orientation_quat_xyzw': [
    #                 float(value)
    #                 for value in orientation_quat
    #             ],
    #             'euler_deg': [
    #                 float(value)
    #                 for value in euler_deg
    #             ]
    #         }
    ...
    #         self.get_logger().error(
    #             f'Unexpected AprilTag callback error: '
    #             f'{exc}'
    #         )
    ...
    def tag_pose_callback(self, msg):
        ...
        try:
            ...
            # ------------------------------------------------
            # 讀取 Camera 對齊後的 Tool7 Pose
            # ------------------------------------------------
            raw_camera_aligned_tool_pose = data.get(
                'camera_aligned_tool7_pose_base'
            )

            if raw_camera_aligned_tool_pose is None:
                raise ValueError(
                    'camera_aligned_tool7_pose_base '
                    'is missing'
                )

            camera_aligned_position_m = (
                raw_camera_aligned_tool_pose.get(
                    'position_m'
                )
            )
            ...
            if (
                camera_aligned_position_m is None
                or len(camera_aligned_position_m) != 3
            ):
                raise ValueError(
                    'camera aligned tool position '
                    'must contain 3 values'
                )

            if (
                camera_aligned_quat is None
                or len(camera_aligned_quat) != 4
            ):
                raise ValueError(
                    'camera aligned tool quaternion '
                    'must contain 4 values'
                )

            if (
                camera_aligned_euler_deg is None
                or len(camera_aligned_euler_deg) != 3
            ):
                raise ValueError(
                    'camera aligned tool Euler '
                    'must contain 3 values'
                )

            camera_aligned_tool_pose = {
                'position_m': [
                    float(value)
                    for value in camera_aligned_position_m
                ],
                'orientation_quat_xyzw': [
                    float(value)
                    for value in camera_aligned_quat
                ],
                'euler_deg': [
                    float(value)
                    for value in camera_aligned_euler_deg
                ]
            }
            ...
            self.camera_aligned_tool_pose_samples.append(
                camera_aligned_tool_pose
            )
            ...
            median_aligned_position = np.median(
                aligned_positions,
                axis=0
            )

            average_aligned_quat = (
                self.average_quaternions(
                    aligned_quaternions
                )
            )

            average_aligned_euler = (
                R.from_quat(
                    average_aligned_quat
                ).as_euler(
                    'xyz',
            ...
            self.latest_camera_aligned_tool7_pose_base = {
                'position_m': (
                    median_aligned_position.tolist()
                ),
                'orientation_quat_xyzw': (
                    average_aligned_quat
                ),
                'euler_deg': (
                    average_aligned_euler.tolist()
                )
            }
