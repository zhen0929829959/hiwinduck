...
from ur_robot_control.insertion_mixin import InsertionMixin
...
PLUG_CENTER_U = 1263.0
PLUG_CENTER_V = 661.0

CAMERA_FX = 1362.7784437304256
CAMERA_FY = 1361.8357978878923

# CAMERA_FX = 1366.140789595595
# CAMERA_FY = 1366.8042320152897
...
# IMPORTANT: replace these with the UR5e photo-pose joint angles in degree.
# Do NOT reuse the old HIWIN joint angles.
#camera6
# PHOTO_POSE = [
#     72.18,       # shoulder_pan_joint
#     -109.52,     # shoulder_lift_joint
#     64.26,       # elbow_joint
#     -80.53,     # wrist_1_joint
#     -78.70,     # wrist_2_joint
#     74.12, # wrist_3_joint
...
PHOTO_POSE = [
    ...
    -80.53,     # wrist_1_joint
    -78.70,     # wrist_2_joint
    -105.88, # wrist_3_joint
]

# TAG_APPROACH_Z_MM = 160.0
# RJ45_APPROACH_Z_MM = 16.0
TAG_APPROACH_Z_MM = 300.0
RJ45_APPROACH_Z_MM = 100.0
...
class ExampleStrategy(
    ...
    def __init__(self):
        ...
        self.tool_pose_pub = self.create_publisher(
            String,
            '/hiwin/tool_pose',  # compatibility topic; payload now comes from UR TCP
            10
        )
    ...
    def _state_machine(self, state):
        ...
        if state == States.MOVE_ABOVE_APRILTAG:
            ...
            self.get_logger().warn(
                f'Tag Z = {tag_z * 1000.0:.2f} mm, '
                f'Target TCP Z = '
                f'{tag_z * 1000.0 + TAG_APPROACH_Z_MM:.2f} mm'
            )
            ...
            self.get_logger().warn(
                f'Actual TCP Z = {current_pose[2]:.2f} mm'
            )
        ...
        if state == States.MOVE_ABOVE_BOARD_CENTER:
            ...
            pose = self.create_pose(
                board_x * 1000.0,
                board_y * 1000.0,
                board_z * 1000.0 + TAG_APPROACH_Z_MM,
                rx,
                ry,
                rz -TARGET_RZ_OFFSET
            )
