...
class ExampleStrategy(
    ...
    def _state_machine(self, state):
        ...
        if state == States.MOVE_ABOVE_APRILTAG:
            ...
            rx, ry, rz = self.photo_orientation_deg
        ...
        if state == States.MOVE_ABOVE_BOARD_CENTER:
            ...
            board_x, board_y, board_z = self.latest_board_center_position_m
            ...
            pose.linear.z = (float(board_z) * 1000.0 + TAG_APPROACH_Z_MM)
        ...
        if state == States.WAIT_YOLO_DETECTION:
            ...
            self.get_logger().info(
                'Waiting 25 seconds for YOLO to stabilize'
            )
